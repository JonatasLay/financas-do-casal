...
function parseStatement(text: string, existingTransactions: Transaction[]) {
  ...
  const duplicateKey = new Set(existingTransactions.map(tx =>
    `${tx.date}|${Math.abs(Number(tx.amount)).toFixed(2)}|${tx.description.trim().toLowerCase()}`
  ))
  ...
  return lines.slice(1).map((line, index) => {
    ...
    return {
      ...
      amount,
      type,
      selected: !!date && amount > 0 && !duplicateKey.has(key),
      duplicate: duplicateKey.has(key),
    } as ImportRow
...
export function StatementImportModal({
  ...
  const handleFile = async (file?: File) => {
    ...
    const parsed = parseStatement(text, existingTransactions)
    setRows(parsed)
  ...
  const saveRows = async () => {
    ...
    const payload = selectedRows.map(row => ({
      ...
      amount: row.amount,
      type: row.type,
      category_id: row.type === 'receita' ? incomeCategoryId || null : expenseCategoryId || null,
      bank_id: bankId || null,
      status: 'realizado',
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center">
      ...
      <div className="relative w-full md:max-w-4xl max-h-[92dvh] rounded-t-3xl md:rounded-2xl flex flex-col"
        ...
        <div className="flex items-center justify-between gap-3 px-5 py-4" style={{ borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
          <div>
            ...
            <p className="text-xs mt-0.5" style={{ color: '#64748B' }}>Use CSV com data, descrição e valor. Linhas duplicadas ficam desmarcadas.</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
          <label className="rounded-2xl p-5 flex flex-col items-center justify-center gap-2 cursor-pointer"
            ...
            <span className="text-xs" style={{ color: '#64748B' }}>Exportação do banco, cartão ou planilha</span>
          ...
          {rows.length > 0 && (
            <>
              ...
              <div className="space-y-2">
                {rows.map(row => (
                  <label key={row.id} className="rounded-xl px-3 py-2.5 flex items-center gap-3 cursor-pointer"
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="text-sm font-semibold truncate" style={{ color: '#F1F5F9' }}>{row.description}</p>
                      <p className="text-xs flex items-center gap-1.5" style={{ color: '#64748B' }}>
                        ...
                        {row.duplicate && <span style={{ color: '#FBBF24' }}>· possível duplicado</span>}
                      </p>
                    </div>
                    <p className="text-sm font-bold" style={{ color: row.type === 'receita' ? '#34D399' : '#F87171' }}>
